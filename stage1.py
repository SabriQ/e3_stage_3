...
#        sys.exit()
    ...
    print(">>>>>>")
    ser_motor.write("444777".encode())#初始化时，给两个lick port出一点水
    print("<<<<<<")
    ...
    with open(log_name, 'w',newline="",encoding='utf-8') as csvfile:
        ...
        writer.writerow(["Trial_Num","A_NosePoke","A_ContextEnter","A_ContextExit","A_Choice","A_ContextREnter","A_ContextRExit",
    "P_NosePoke","P_ContextEnter","P_ContextExit","P_Choice","P_ContextREnter","P_ContextRExit"])
    print(["Trial_Num","Nose_poke","Choice"])
    ...
    while True:
        ...
        if len(info)>1:
            ...
            if "Sum:" in info:
                ...
                if Trial_Num[-1] !="0":
                    row=[Trial_Num[-1],A_NosePoke[-1],A_ContextEnter[-1],A_ContextExit[-1],A_Choice[-1],A_ContextREnter[-1],A_ContextRExit[-1],P_NosePoke[-1],P_ContextEnter[-1],P_ContextExit[-1],P_Choice[-1],P_ContextREnter[-1],P_ContextRExit[-1]]
                    with open(log_name,"a",newline="\n",encoding='utf-8') as csvfile:
                        writer = csv.writer(csvfile)
                        writer.writerow(row)
                    print(row[0],'[',round(row[7],2),round(row[10],2),']')
                else:
                    print("unfinished trial terminated")
            if "Sum" in show_info:
            	show_info = "Ready "
