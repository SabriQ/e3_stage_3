...
//  Serial.println(on_signal);
  ...
  if (rl < 500 && rl >50){ir[3] = 0;}else{ir[3] = 0;}  
  if (rr < 900 && rr >5){ir[4] = 1;}else{ir[4] = 0;}  
  if (rs < 200 ){ir[5] = 1;}else{ir[5] = 0;}   
...
//  ir[5] ir_rs // context exit or reverse_enter  
    ...
    do{Read_ir();}while(on_signal>0.5 && ir[2]==0);unsigned long context_enter_time = millis();
    Serial.println("Stat2: context_enter");
    do{Read_ir();}while(on_signal>0.5 && ir[5]==0);unsigned long context_exit_time = millis();
    Serial.println("Stat3: context_exit");
    do{Read_ir();}while(on_signal>0.5 && ir[3]==0 && ir[4]==0 );unsigned long choice_time = millis();
    ...
    do{Read_ir();}while(on_signal>0.5 && ir[5]==0);unsigned long context_reverse_enter_time = millis();
    Serial.println("Stat5: contextRenter");
    do{Read_ir();}while(on_signal>0.5 && ir[2]==0);unsigned long context_reverse_exit_time = millis();
